...
export function drawRobot(el: HTMLElement, state: AgentState) {
  // Brighter steel with the theme's amber as the accent, so it reads as a
  // different kind of creature from the fly without leaving the palette.
  const lit = state === 'think' ? '#7defd9' : state === 'happy' ? '#a6ef7d'
            : state === 'sad' ? '#f2a184' : state === 'nap' ? '#4f7480' : '#8ddcf2';
  ...
  <svg viewBox="0 0 112 92" width="100%" height="100%" role="img" aria-label="robot">
    ...
    <g transform="rotate(${tilt} 56 56)">
      <line x1="56" y1="22" x2="56" y2="14" stroke="#8fb3bd" stroke-width="3"/>
      <circle cx="56" cy="11" r="4.5" fill="#f0b54a">
        ${blink !== '0s' ? `<animate attributeName="opacity" values="1;.25;1" dur="${blink}" repeatCount="indefinite"/>` : ''}
      </circle>
      <rect x="26" y="22" width="60" height="52" rx="11" fill="#5f8892"/>
      <rect x="26" y="22" width="60" height="18" rx="11" fill="#7aa4ae" opacity=".55"/>
      <rect x="30" y="26" width="52" height="34" rx="8" fill="#183840"/>
      ${eyes}
      <path d="${mouth}" stroke="#cfeef6" stroke-width="2.6" fill="none" stroke-linecap="round"/>
      <rect x="18" y="38" width="7" height="18" rx="3" fill="#8fb3bd"/>
      <rect x="87" y="38" width="7" height="18" rx="3" fill="#8fb3bd"/>
      <rect x="46" y="64" width="20" height="4" rx="2" fill="#f0b54a" opacity=".75"/>
